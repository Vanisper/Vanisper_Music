...
import { useGetState, useToggle, useCreation, useUpdateEffect } from "ahooks";
...
import { secondToMinute } from "~/utils/date/index";
import {
  SortOne,
  PlayCycle,
  ShuffleOne,
      ...
      <ul className="flex w-full justify-center items-center space-x-5">
        ...
        <li>
          {isPause ? (
            <button
              data-tip="暂停"
              className="tooltip btn btn-md btn-circle"
              onClick={handlePause}
            >
        ...
        <li>
          <span
            className={`cursor-pointer ${
              state.showLyric === true ? "text-purple-500" : ""
