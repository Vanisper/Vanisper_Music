...
const SearchResult: FunctionComponent<SearchResultProps> = () => {
  ...
  const [searchSuggestList, setSearchSuggestList, getSuggestList] =
    useGetState<SearchSuggestType>({
      albums: [],
      artists: [],
      songs: [],
    }); // 搜索到的推荐
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // // 获取建议的搜索结果  单曲---歌手---专辑
      let res1: any = await getSearchSuggestList({
        keywords: state.searchWords,
      });
      if (res1.code === 200) {
      ...
      <ul className=" mt-3  w-full ">
        {searchLimitList.map((data) => {
          return (
            <li
              key={data.id}
              className="h-8 truncate flex items-center cursor-pointer px-3 text-xs hover:bg-gray-700 rounded-md"
            >
          ...
          <ul className="w-full">
            {getSuggestList().artists.map((data) => {
              return (
                <li
                  key={data.id}
                  className="py-3 truncate flex items-center cursor-pointer px-3 text-xs hover:bg-gray-700 rounded-md"
                >
                  <img
                    src={data.img1v1Url}
                    className="w-7 h-7 object-contain rounded-md mr-3"
                  />
                  {<span className="text-green-600">{data.name}</span>}
                </li>
              );
